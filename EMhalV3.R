...
fit_cv_hal_joint <- function(data_in, folds,
  ...
  cv_nll_sum <- outer(rowMeans(cv_nll_cen), rowMeans(cv_nll_out), "+")
...
gcomp.nde <- function(data) {
  ...
  nde.rd <- mean(y1 - y0)
  nde.rr <- mean(y1) / mean(y0)
  nde.or <- (mean(y1) / (1 - mean(y1))) / (mean(y0) / (1 - mean(y0)))
...
gcomp.nie <- function(data) {
  ...
  # Calculate and return the estimate
  nid.rd <- mean(y1 - y0)
  nid.rr <- mean(y1) / mean(y0)
  nid.or <- (mean(y1) / (1 - mean(y1))) / (mean(y0) / (1 - mean(y0)))
